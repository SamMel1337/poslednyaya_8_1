...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'username', 'email', 'password', 'password_confirm',
                  'first_name', 'last_name', 'telegram_chat_id', 'phone')
        read_only_fields = ('id',)
...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'username', 'email', 'first_name', 'last_name',
                  'telegram_chat_id', 'phone', 'is_telegram_connected', 'date_joined')
...
class TelegramConnectionSerializer(serializers.Serializer):
    ...
    def validate_telegram_chat_id(self, value):
        if User.objects.filter(telegram_chat_id=value).exclude(id=self.context['user'].id).exists():
            raise serializers.ValidationError("Этот Telegram ID уже используется другим пользователем")
        return value